...
6. **Visual Resumption (Canvas Runs History)**: The operator opens **`[📜 Runs History]`** in Canvas, views the paused execution with Stage 1 and Stage 2 intact, and clicks **`▶️ Resume DAG Execution`** (`POST /api/canvas/resume/{run_id}`).
