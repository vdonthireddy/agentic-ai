...
| **`[13]`** | **System Config & Health View** | [webui/src/views/SettingsView.jsx:L1-L350](file:///Users/donthireddy/code/github/agentic-ai/webui/src/views/SettingsView.jsx#L1-L350) | Configuration interface for managing active models, fallbacks, API keys, system prompts, runtime hyperparameters (including HITL timeout), and checking endpoint liveness. |
| **`[13b]`** | **Safety Approvals View (HITL Hub)** | [webui/src/views/ApprovalsView.jsx:L1-L598](file:///Users/donthireddy/code/github/agentic-ai/webui/src/views/ApprovalsView.jsx#L1-L598) | Dedicated control tower for managing Human-in-the-Loop approvals: pending request queue cards, arguments JSON inspector, real-time auto-denial countdown timer (20-minute default / 0 for infinite), safety rules registry, and audit resolution ledger. |
| **`[14]`** | **HITL Approval Modal Component** | [webui/src/components/HITLApprovalModal.jsx:L1-L146](file:///Users/donthireddy/code/github/agentic-ai/webui/src/components/HITLApprovalModal.jsx#L1-L146) | Non-blocking modal that intercepts destructive agent actions (file deletes, DB writes) and prompts the human user for explicit approval with formatted countdown and infinite window support. |
...
| **`[15e]`** | **Sidebar Navigation** | [webui/src/components/Sidebar.jsx:L1-L100](file:///Users/donthireddy/code/github/agentic-ai/webui/src/components/Sidebar.jsx#L1-L100) | Primary left-rail navigation component managing tab routing across all 12 studio views with real-time pending approvals badge. |
| **`[15f]`** | **Top Header Bar** | [webui/src/components/TopHeader.jsx:L1-L80](file:///Users/donthireddy/code/github/agentic-ai/webui/src/components/TopHeader.jsx#L1-L80) | Studio header component displaying branding, connection status, active model indicator, and global glowing HITL approval alert badge. |
| **`[15g]`** | **API Client Layer** | [webui/src/api/client.js:L1-L320](file:///Users/donthireddy/code/github/agentic-ai/webui/src/api/client.js#L1-L320) | Centralized HTTP/fetch client module providing typed API helper functions for all `/api/*` and `/v1/*` gateway endpoints. |
...
| **`[33]`** | **HITL Safety Interceptor** | [mcp_server/hitl.py:L1-L365](file:///Users/donthireddy/code/github/agentic-ai/mcp_server/hitl.py#L1-L365) | Security layer with `RiskLevel` enum (LOW/MEDIUM/HIGH/CRITICAL), default 20-minute safety time limit (1200s), deterministic auto-denial on expiration, support for 0 as infinite, async `wait_for_resolution`, and approval/denial history tracking. |
...
| **Zombie Stalled Requests** | If an operator walks away, workflows block indefinitely in a frozen state. | **20-Minute Enforced Timeout & Auto-Denial**: Unanswered requests auto-deny after 20 minutes (1200s), with `0` supporting an infinite window. |
